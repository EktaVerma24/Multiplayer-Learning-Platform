...
import toast, { Toaster } from "react-hot-toast"; // 👈 UPDATED: Import Toaster
...
        function onAllCountsUpdate(allCounts) { 
...
    // --- NEW FUNCTION: Handles the API call after confirmation ---
...
            toast.error("Could not delete classroom. Please try again."); 
            setError("Could not delete classroom. Please try again.");
        }
    }

    // --- UPDATED handleDelete: Uses custom toast for confirmation ---
    const handleDelete = (id) => {
        const classroomToDelete = classrooms.find(cls => cls._id === id);
        if (!classroomToDelete) return;

        // Interactive confirmation toast
        toast((t) => (
            <div className="bg-white p-4 rounded-lg shadow-2xl border border-red-300 w-80">
                <p className="text-gray-800 font-semibold mb-3">
                    Are you sure you want to delete <span className="text-red-600 font-bold">"{classroomToDelete.name}"</span>?
                </p>
...
                        onClick={async () => {
                            toast.dismiss(t.id);
                            await performDelete(id, classroomToDelete.name);
                        }}
...
            style: { padding: 0, border: 'none', background: 'transparent' } // Remove default styling for custom content
        });
    }

    // --- handleCreateClassroom: Contains success/error toasts ---
...
            toast.success(`Classroom "${res.data.name}" created successfully!`, {
                icon: '🎉',
                style: {
...
            toast.error(`Error creating classroom: ${serverErrorMessage}`);
        } finally {
...
        if (!syllabus.trim()) {
            toast.error("Enter a syllabus or topic first.");
            return;
        } 
        try {
...
            toast.error("Could not prepare notes for download.");
            return;
        }
...
        if (isBanned) { 
            toast.error("You have been banned from this classroom.");
            return;
        } else {
...
            } else {
                toast.error("You are not eligible to create a quiz at this time.");
            }
        } catch (err) {
            console.error("Failed to check quiz eligibility:", err);
            toast.error("Could not check quiz eligibility. Please try again.");
        }
    };
    
...
            {/* 🎯 TOASTER COMPONENT: This must be present for toasts to render */}
...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        handleDelete(cls._id); // Call the toast-based handler
                                                        }}
                                                    className="absolute top-0 right-0 cursor-popinter p-1 rounded-full text-slate-400 hover:bg-red-100 hover:text-red-600 transition-colors duration-300"
...
                                                        {section.imageUrl && (
                                                                    <img src={`${import.meta.env.VITE_BACKEND_URL}/image-proxy?url=${encodeURIComponent(section.imageUrl)}`} alt={section.title} className="my-2 max-w-sm h-auto rounded-md border" />
                                                                )}
