...
export default function Dashboard({ user }) {
    ...
    const handleDownloadPDF = async (note) => {
        ...
        try {
            const canvas = await html2canvas(noteContent, { scale: 2, backgroundColor: '#ffffff', useCORS: true });
                                ...
                                <div className="p-6 cursor-pointer group flex flex-col justify-between h-full" onClick={() => enterClassroom(cls._id)}>
                                    ...
                                    {user.role === "teacher" && (
                                        <div className="mt-6 pt-4 border-t flex gap-3">
                                            <button onClick={(e) => { e.stopPropagation(); goToCreateQuiz(cls._id); }} className="flex items-center justify-center w-full px-4 py-2 font-semibold text-white bg-violet-600 rounded-md hover:bg-violet-700">
                                                <QuizIcon /> Create Quiz
                                            </button>
                                            <button onClick={(e) => { e.stopPropagation(); goToCreateChallenge(cls._id); }} className="flex items-center justify-center w-full px-4 py-2 font-semibold text-white bg-purple-500 rounded-md hover:bg-purple-600">
