...
export default function ClassroomPage({ user }) {
  ...
  const LoadingSpinner = () => (
    <div className="h-screen flex justify-center items-center p-10">
      <div className="w-16 h-16 border-4 border-dashed rounded-full animate-spin border-violet-500" />
    </div>
  );
  ...
  useEffect(() => {
    ...
    const handleUsersInChat = ({ users = [] }) => {
      setUsersInChat(users);
    };

    socket.on("usersInChat", handleUsersInChat);
    return () => socket.off("usersInChat", handleUsersInChat);
  }, [socket]);

  useEffect(() => {
    if (!socket || !user?._id) return;

    const handleKicked = ({ message, userId }) => {
  ...
  useEffect(() => {
    if (!socket || !user?._id || !classroomId) return;
  ...
  const handleToggleChatPause = () => {
    if (!socket || !adminChatAccess || !classroomId) return;
    const nextPaused = !paused;
    setPaused(nextPaused);
    socket.emit("chatPause", { classroomId, paused: nextPaused });
  };

  const handleKickUser = async (userToKick) => {
    if (!adminChatAccess || !userToKick || !socket || !classroomId) return;
  ...
  useEffect(() => {
    ...
    const handleChatPaused = ({ paused: isPaused }) => {
      setPaused(Boolean(isPaused));
    };

    socket.on("chatPausedByAdmin", handleChatPaused);
    return () => socket.off("chatPausedByAdmin", handleChatPaused);
  }, [socket]);
  ...
  const sendMessage = (e) => {
    e.preventDefault();
    if (!input.trim() || !socket || !user?._id || !classroomId) return;
    ...
    socket.emit("sendMessage", {
      ...
      replyTo: replyingTo
        ? {
            ...
            content: replyingTo.message.substring(0, 100),
          }
        : null,
    });
  ...
  const canSendMessage = !paused || adminChatAccess;
  const activeNames = usersInChat.map((member) => member?.name).filter(Boolean).slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <a
          href="/dashboard"
          className="absolute top-4 right-6 p-2.5 rounded-full bg-violet-100 transition-all duration-300 shadow-sm hover:shadow-md"
        >
          <FaHome size={20} />
        </a>
        ...
        <div className="border-b border-slate-300 flex items-center">
          ...
          <TabButton active={tab === "whiteboard"} onClick={() => handleTabChange("whiteboard")} icon={<WhiteboardIcon />}>
            Whiteboard
          </TabButton>
          <TabButton active={tab === "quiz"} onClick={() => handleTabChange("quiz")} icon={<QuizIcon />}>
            Quizzes
          </TabButton>
          <TabButton active={tab === "challenge"} onClick={() => handleTabChange("challenge")} icon={<ChallengeIcon />}>
            Challenges
          </TabButton>
          <TabButton active={tab === "video"} onClick={() => handleTabChange("video")} icon={<VideoIcon />}>
            Video Call
          </TabButton>
          <TabButton active={tab === "availablejobs"} onClick={() => handleTabChange("availablejobs")} icon={<JobsIcon />}>
            Jobs
          </TabButton>
        </div>
          ...
          {tab === "chat" && (
            <div className="flex flex-col h-[420px]">
              <div className="flex items-center justify-between mb-3 text-sm text-slate-500">
                <p>
                  Active participants: {usersInChat.length}
                  {activeNames.length > 0 && ` • ${activeNames.join(", ")}${
                    usersInChat.length > activeNames.length ? "..." : ""
                  }`}
                </p>
                {adminChatAccess && (
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-slate-600">{paused ? "Chat paused" : "Chat live"}</span>
                    <button
                      type="button"
                      aria-pressed={paused}
                      onClick={handleToggleChatPause}
                      className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-300 focus:outline-none ${
                        paused ? "bg-slate-300" : "bg-violet-600"
                      }`}
                    >
                      <span
              ...
              <div className="flex-grow p-4 space-y-4 overflow-y-auto bg-slate-50 rounded-lg">
                {messages.length === 0 && <p className="text-slate-400">No messages yet. Start the conversation!</p>}
                {messages.map((msg, index) => {
                  if (!msg || !msg.user) return null;

                  const isCurrentUser = msg.user._id === user?._id;
                  const messageKey = `${msg._id || msg.timestamp || index}-${index}`;
                  ...
                  return (
                    ...
                    >
                      {!isCurrentUser && (
                        <UserAvatarWithKick user={msg.user} onKick={handleKickUser} isAdmin={adminChatAccess} />
                      )}
                      <div
                      ...
                      >
                        {!isCurrentUser && (
                          <p className="text-xs font-bold text-violet-700 mb-1">{msg.user?.name || "Member"}</p>
                        )}
                        {msg.replyTo && (
                          <div className="mb-2 rounded-lg bg-white/30 px-3 py-1 text-xs text-slate-700">
                        ...
                        {!isCurrentUser && (
                          <button
                            ...
                            className="mt-2 text-xs underline hover:opacity-80 transition"
                          >
                            Reply
                          </button>
                        )}
              ...
              {paused && !adminChatAccess && (
                <p className="mt-3 text-center text-sm font-semibold text-red-500">Chat paused by admin</p>
              )}

              {replyingTo && (
                <div className="mt-3 w-full px-4 py-2 bg-violet-50 border border-violet-200 rounded-lg flex items-center justify-between">
                  <div className="flex-1">
                    <p className="text-xs font-semibold text-violet-700">
                      Replying to {replyingTo.user?.name || "user"}
                    </p>
                    <p className="text-xs text-slate-600 truncate">{replyingTo.message}</p>
                  </div>
                  <button
                    type="button"
              ...
              <form onSubmit={sendMessage} className="mt-4 flex items-center gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 border border-slate-300 rounded-full shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 transition"
                ...
                <button
                  type="submit"
                  disabled={!canSendMessage}
                  className={`flex-shrink-0 p-3 rounded-full text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-300 shadow-md ${
                    canSendMessage ? "bg-violet-600 hover:bg-violet-700 focus:ring-violet-500" : "bg-slate-400"
                  }`}
                >
                  <SendIcon />
                </button>
